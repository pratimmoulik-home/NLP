...
Original file is located at
    https://colab.research.google.com/drive/1W084gbV6wcSgfYFr56VIWoDEvzmhqOOC
...
class RnnDecoder(nn.Module):
    ...
    def compute_attention(self, dec_hs, enc_output):
        ...
        first = self.w_1(dec_hs).permute(1,0,2)
        second = self.w_2(enc_output).permute(1,0,2)
        temp = torch.tanh(first + second)
        scores = self.w_3(temp)
        attention_weights = F.softmax(scores, dim=1)
        context_vector = torch.sum(attention_weights * enc_output.permute(1,0,2), dim=1)
    ...
    def forward(self, x, dec_hs, enc_output):
        ...
        concatenated_output = torch.cat((context_vector, embed_vector.squeeze(1)), dim=1)
        concatenated_output = concatenated_output.unsqueeze(1)
        fc_out, dec_hs = self.gru(concatenated_output, dec_hs)
        fc_out = self.final_layer(fc_out)
...
def decode_rnn_model(encoder, decoder, src, max_decode_len, device):
    ...
    for i in range(1, max_decode_len):
      dec_output, hidden_state, attention_weights = decoder(dec_input, hidden_state,enc_output)
      curr_predictions[:, i] = dec_output
      dec_input = torch.argmax(curr_predictions[:, i], dim=-1).unsqueeze(1)
      curr_output[:, i] = dec_input.squeeze(1)
    ...
    for pos in range(max_len):
      ...
      for j in range(embed_dim):
        ...
        if(j%2 == 0):
          ...
          f = j-1
        denom = math.exp((f/embed_dim)*math.log(10000))
        frac = pos/denom
        if(j%2 == 0):
          temp.append(math.sin(frac))
        else:
          temp.append(math.cos(frac))
...
class TransformerDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, num_heads,
        ...
        self.dropout_layer = nn.Dropout(dropout)
        self.trans_decoder = nn.TransformerDecoder(nn.TransformerDecoderLayer(embedding_dim, num_heads, dim_feedforward), num_layers)
        self.final_layer = nn.Linear(embedding_dim, trg_vocab_size)
    ...
    def forward(self, dec_in, enc_out):
        ...
        ### TODO ###
        output = self.embedding(dec_in)
        output = output + self.positional_embedding[:dec_in.size(0), :]
        output = self.dropout_layer(output)
        trg_mask = self.generate_square_subsequent_mask(dec_in.size(0))
        output = self.trans_decoder(output, memory=enc_out, tgt_mask=trg_mask)
...
def decode_transformer_model(encoder, decoder, src, max_decode_len, device):
    ...
    for i in range(1, max_decode_len):
      dec_input = curr_output[:, :i].long().permute(1,0)
      dec_output = decoder(dec_input, enc_output)
      curr_predictions[:, i] = dec_output[-1]
      curr_output[:, i] = torch.argmax(curr_predictions[:, i], dim=-1)
